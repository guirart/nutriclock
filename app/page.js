...
import {
  ...
  Trash2, ChevronLeft, ChevronRight, CalendarDays, Sword, Shield, Crown, Trophy, LockKeyhole, Sparkles, PackageOpen, Backpack, Gem, Coins, Shirt, Footprints, CircleDot, LogOut, Mail, KeyRound, Camera, Send, Bot, Image as ImageIcon, Check, X
} from "lucide-react";
import {
...
const DAILY_BOSSES = [
  {id:"skeleton-scout",name:"Sentinela Esqueleto",environment:"Clareira do Café",enemy:"skeleton1",maxHp:180,reward:{bananas:4,gems:2,chest:"rare"},story:"Uma sentinela antiga ocupou a clareira e protege cristais escondidos."},
  {id:"skeleton-knight",name:"Cavaleiro Ósseo",environment:"Terraço Abandonado",enemy:"skeleton2",maxHp:210,reward:{bananas:5,gems:3,chest:"rare"},story:"O cavaleiro ergueu sua espada e bloqueou a passagem para o próximo acampamento."},
  {id:"vampire-lord",name:"Lorde Vampiro",environment:"Café da Meia-Noite",enemy:"vampire",maxHp:250,reward:{bananas:6,gems:5,chest:"epic"},story:"O lorde noturno drena a energia da jornada e guarda uma gema rara."},
  {id:"bone-guardian",name:"Guardião de Ossos",environment:"Clareira do Café",enemy:"skeleton2",maxHp:230,reward:{bananas:5,gems:4,chest:"epic"},story:"Um guardião despertou entre as mesas antigas e desafia sua disciplina."},
  {id:"crypt-runner",name:"Corredor da Cripta",environment:"Terraço Abandonado",enemy:"skeleton1",maxHp:200,reward:{bananas:4,gems:3,chest:"rare"},story:"Rápido e imprevisível, ele só recua diante de uma sequência de bons hábitos."},
  {id:"night-count",name:"Conde da Noite",environment:"Café da Meia-Noite",enemy:"vampire",maxHp:280,reward:{bananas:7,gems:6,chest:"legendary"},story:"O inimigo mais poderoso da semana diária apareceu para testar sua constância."},
  {id:"ancient-skeleton",name:"Esqueleto Ancestral",environment:"Clareira do Café",enemy:"skeleton1",maxHp:240,reward:{bananas:6,gems:5,chest:"epic"},story:"Uma armadura vazia voltou a andar e tomou conta da clareira principal."}
];
...
function normalizeAssetSource(value){
  ...
  if(clean.includes("/game/")) return clean.slice(clean.indexOf("/game/"));
  return "";
}

function ItemIcon({item,className="assetItemIcon"}){
  const rawSource=typeof item==="string"?item:item?.icon;
  const source=normalizeAssetSource(rawSource);
  const label=typeof item==="string"?"Item":item?.name||"Item";
  if(source) return <span className={`${className} assetIconFrame`} aria-label={label}><img src={encodeURI(source)} alt="" draggable="false" onError={event=>{event.currentTarget.style.display="none";event.currentTarget.parentElement?.classList.add("assetIconMissing")}}/></span>;
  return <span className={`${className} assetIconFrame assetIconMissing`} aria-label={label}/>;
}
...
function NutriAssist({session,onRegistered}){
  ...
  async function chooseFile(event){
    ...
    reader.onload=()=>{setPreview(String(reader.result));setAnalysis(null);setStatus("");};
    reader.readAsDataURL(file);
  }

  async function analyze(){
    if(!preview&&!message.trim()){setStatus("Envie uma foto ou escreva uma descrição.");return;}
    setLoading(true);setStatus("Analisando refeição...");
    try{
      const response=await fetch("/api/ai/analyze-meal",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${session.access_token}`},body:JSON.stringify({image:preview,message})});
      const data=await response.json();
      if(!response.ok) throw new Error(data.details||data.error||"Falha na análise.");
      setAnalysis(data.analysis);setStatus("Confira a estimativa antes de registrar.");
    }catch(error){setStatus(error.message);}finally{setLoading(false);}
  }

  async function registerMeal(){
    if(!analysis) return;
    setLoading(true);setStatus("Registrando refeição...");
    try{
      const entry={type:"meal",description:analysis.meal_name||"Refeição analisada",calories:Math.round(analysis.totals.calories||0),protein_g:Number(analysis.totals.protein_g||0),carbs_g:Number(analysis.totals.carbs_g||0),fat_g:Number(analysis.totals.fat_g||0),fiber_g:Number(analysis.totals.fiber_g||0),confidence:analysis.confidence||"medium",notes:[...(analysis.warnings||[]),`Alimentos: ${(analysis.foods||[]).map(f=>`${f.name} (${Math.round(f.estimated_grams)} g)`).join(", ")}`].join(" | "),source:"nutriassist",occurred_at:new Date().toISOString()};
      const response=await fetch("/api/entries",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${session.access_token}`},body:JSON.stringify(entry)});
      const data=await response.json();
      if(!response.ok) throw new Error(data.details||data.error||"Falha ao registrar.");
      setStatus("Refeição registrada com sucesso.");setAnalysis(null);setPreview("");setMessage("");
      onRegistered?.();
    }catch(error){setStatus(error.message);}finally{setLoading(false);}
  }

  return <section className="page nutriAssistPage">
    <div className="hero assistHero"><Bot size={42}/><div><span>NutriAssist</span><h2>Fotografe sua refeição e registre sem sair do app</h2><p>A IA estima alimentos, porções, calorias e macronutrientes. Você confirma antes de salvar.</p></div></div>
    <div className="assistLayout">
      <article className="panel assistComposer">
        <div className="panelHead"><div><h2>Nova análise</h2><p>Use uma foto nítida e descreva detalhes invisíveis, como óleo, molho ou recheio.</p></div></div>
        <input ref={fileRef} type="file" accept="image/*" capture="environment" hidden onChange={chooseFile}/>
        <button className="assistUpload" type="button" onClick={()=>fileRef.current?.click()}>
          {preview?<img src={preview} alt="Prévia da refeição"/>:<><Camera size={40}/><strong>Tirar ou enviar foto</strong><small>JPG, PNG ou WEBP · até 8 MB</small></>}
        </button>
        {preview&&<button className="assistRemove" type="button" onClick={()=>{setPreview("");setAnalysis(null)}}><X size={16}/> Remover foto</button>}
        <label className="assistPrompt">Observações<textarea value={message} onChange={e=>setMessage(e.target.value)} placeholder="Ex.: tinha queijo por baixo, o molho era maionese e a porção de arroz estava grande."/></label>
        <button className="primary assistAnalyze" type="button" disabled={loading} onClick={analyze}><Send size={18}/>{loading?"Processando...":"Analisar refeição"}</button>
        {status&&<p className="assistStatus">{status}</p>}
      </article>
      <article className="panel assistResult">
        {!analysis?<div className="assistEmpty"><ImageIcon size={46}/><h3>A análise aparecerá aqui</h3><p>O registro só será criado depois da sua confirmação.</p></div>:<>
          <div className="panelHead"><div><span className="assistBadge">Confiança {analysis.confidence}</span><h2>{analysis.meal_name}</h2><p>{analysis.assistant_message}</p></div></div>
          <div className="assistTotals"><div><strong>{Math.round(analysis.totals.calories)}</strong><span>kcal</span></div><div><strong>{Math.round(analysis.totals.protein_g)} g</strong><span>proteína</span></div><div><strong>{Math.round(analysis.totals.carbs_g)} g</strong><span>carboidratos</span></div><div><strong>{Math.round(analysis.totals.fat_g)} g</strong><span>gorduras</span></div></div>
          <div className="assistFoods">{analysis.foods.map((food,index)=><div key={`${food.name}-${index}`}><div><strong>{food.name}</strong><small>{Math.round(food.estimated_grams)} g · confiança {food.confidence}</small></div><b>{Math.round(food.calories)} kcal</b></div>)}</div>
          {!!analysis.warnings?.length&&<div className="assistWarnings"><strong>Atenção</strong>{analysis.warnings.map((warning,index)=><p key={index}>{warning}</p>)}</div>}
          <button className="primary assistConfirm" type="button" disabled={loading} onClick={registerMeal}><Check size={18}/>Confirmar e registrar</button>
        </>}
      </article>
    </div>
  </section>;
...
{active==="home"&&<>
    ...
    {active==="assistant"&&<NutriAssist session={session} onRegistered={load}/>}
    ...
    <nav className="bottomNav">
      ...
      <button className={active==="assistant"?"active":""} onClick={()=>setActive("assistant")}><Bot/><span>Assistente</span></button>
